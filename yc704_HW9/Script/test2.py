...
def get_image_path():
	...
	facedata_folder = os.listdir("FACESdata_test2")#s1-s40 as list
	...
	#read s1 - s40
		image_path = path + "/FACESdata_test2" + "/" + facedata_folder[i]
...
def test1():
	...
	path = os.getcwd()
	path = path + '/test2/'
	test1Image = Image.open(path + 'TEST_image.jpg').convert('L')
	...
	k = int(input("Enter an k eigenvalue <= 400 for test 2: "))
	while (int(k)>400):
		print('\nMaximum k is 400, please re-enter!')
		k = int(input("Enter k <= 400 for test 2: "))
